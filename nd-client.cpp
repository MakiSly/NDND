...
//0928
      addFace(ss.str(),false,name.toUri());
  ...
  void onAddFaceDataReply(const Interest& interest, const Data& data,
                          const string& uri, bool is_server_face, const string& prefix) 
  {
    short response_code;
    char response_text[1000] = {0};
...
/////////////////////////////////////////////////////////////
	...
	std::cout<<"Founded Name"<<it->second<<"yet name"<<prefix<<"interest"<<interest<<std::endl;
	Name name(prefix);
	registerRoute(name, face_id, 0, is_server_face);
...
//0928
  void addFace(const string& uri, bool is_server_face = false, const string& prefix = "") 
  {
    printf("NDND (Client): Adding face: %s\n", uri.c_str());
    Interest interest = prepareFaceCreationInterest(uri, m_keyChain,0);
    m_face.expressInterest(
      interest,
      bind(&NDNDClient::onAddFaceDataReply, this, _1, _2, uri, is_server_face, prefix),
      bind(&NDNDClient::onNack, this, _1, _2),
      bind(&NDNDClient::onTimeout, this, _1));
  }
...
main(int argc, char** argv)
{
  ...
  */
  ...
  try {
    ...
    po::notify(vm);
  ///////////////////////////////////////////////// 
  std::cout<<"given server_ip"<<opt.server_ip<<std::endl;
  Program program(opt);
